...
export default function RunnerFormCard({ horse }: RunnerFormCardProps) {
  ...
  return (
    <div className='w-[150px] bg-white  p-3 shadow-xl'>
      {/* Header */}
      <div className='flex flex-col items-start gap-2 mb-2'>
        <Image src={horse.image} alt={horse.name} width={40} height={40} />
      ...
      <div className='text-green-600 text-sm mb-3'>
        {'★'.repeat(horse.rating)}
        <span className='text-gray-300'>{'★'.repeat(5 - horse.rating)}</span>
      </div>

      {/* Lengths Behind Winner */}
      <div className='space-y-3 relative pr-6'>
        {last5.map((len, i) => (
          <div key={i} className='flex items-center gap-2 mb-7'>
            {/* Segments + SVG */}
            <div className='flex-1 relative'>
              {/* Segmented bar */}
              <div
                className='absolute -top-[400%] flex items-center justify-center text-xs bg-gray-600 text-white w-4 h-3 font-semibold'
                style={{
                  left: `calc(${(8 - len) * 12.5}% - 4px)`,
                }}
              >
                {len}
              </div>
              <div className='grid grid-cols-8 h-[3px]'>
                {Array.from({ length: 8 }).map((_, idx) => (
                  <div key={idx} className='h-full bg-green-600' />
                ))}
              </div>

              {/* SVG aligned under active segment */}
              <svg
                xmlns='http://www.w3.org/2000/svg'
                viewBox='0 0 67.5 18.1'
                className='absolute top-[6px] w-8 h-4 text-gray-700'
                fill='currentColor'
                style={{
                  left: `calc(${(8 - len) * 12.5}% - 8px)`,
                }}
              >
              ...
              {/* Numeric value directly under SVG */}
            </div>
        ...
        {/* Vertical label */}
        {/* <div className='absolute right-0 top-1/2 -translate-y-1/2 rotate-90 text-xs text-gray-400 whitespace-nowrap'>
          Lengths Behind Winner
        </div> */}
      </div>
    </div>
