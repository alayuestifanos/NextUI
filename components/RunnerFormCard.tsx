...
export default function RunnerFormCard({ horse, image }) {
  const last5 = horse["Form"].split(",").map(Number);

  return (
    <div className="w-[170px] bg-white p-2  border border-gray-100 relative">
      {/* Header */}
      <div className="flex flex-col items-start gap-2 mb-1 pr-4">
      ...
      <div className="space-y-2 pr-10 relative">
        {last5.map((len, i) => (
          <div key={i} className="relative h-6">
            ...
            <div className="absolute inset-0 flex items-center pr-6">
              <div className="h-[2px] bg-green-600" style={{ width: `100%` }} />
            </div>
