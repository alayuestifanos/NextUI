    ...
    <>
      <div className="flex gap-2 max-w-full overflow-x-auto">
        <table className="w-full min-w-[600px] table-auto border-collapse">
          <thead className="text-xs font-normal uppercase text-gray-500">
            <tr>
              ...
              <th className="w-14 px-0.5  text-center font-normal text-black">
                RATING
              </th>
              <th className="w-16 px-0.5  text-left font-normal text-black">
                LAST 5
              </th>
              <th className="w-20 px-1  text-center font-normal text-black">
                WIN
              </th>
              <th className="w-20 px-1  text-center font-normal text-black">
                PLACE
              </th>
              <th className="w-20 px-1 py-2 text-center font-normal text-black">
                COMBO
              </th>
              <th className="w-20 px-1  text-center font-normal text-black">
                BANK
          ...
          <tbody>
            {horses.map((horse, index) => {
              ...
              const horseImage = images[imageIndex];
              return (
                <tr
                  key={horse["FeedId"]}
                  className="border-b border-[rgba(55,179,74,0.1)] text-sm px-1 py-2"
                >
                  ...
                  <td className="px-3 w-[20%] font-normal text-base text-[#727272] whitespace-nowrap">
                    {horse["Name"]}
                  </td>
                  ...
                  <td
                    ...
                    onMouseEnter={(e) => {
                      ...
                      setHovered({ horse, rect, image: horseImage });
                    }}
                  ...
                  <td className="m-0 px-[3px] align-middle ">
                    <div className="flex gap-0 text-xs text-gray-500">
                      {horse["Form"].split(",").map((num, i, array) => (
                        <span key={i}>
                          <span className="cursor-pointer m hover:text-green-600">
                            {num}
                          </span>
                          {i < array.length - 1 && (
                  ...
                  <td className="m-0 px-[3px] align-middle  ">
                    <BankBadge
                      value={horse["WinOdds"]}
                      className={
                        type === "past" || type === "next" ? "opacity-100" : ""
                      }
                      showLock={type !== "next"}
                    ...
                    <BankBadge
                      ...
                      className={
                        type === "past"
                          ? "opacity-100"
                          : type === "next"
                          ? "opacity-100 hover:bg-[#36B250] hover:text-white"
                          : ""
                      }
                      showLock={type !== "next"}
                      disabled={type === "next" ? false : true}
      ...
      {hovered &&
        ...
        createPortal(
          ...
          >
            <RunnerFormCard horse={hovered.horse} image={hovered.image} />
          </div>,
          document.body
        )}
    </>
