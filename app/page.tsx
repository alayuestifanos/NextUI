"use client";
import { useState } from "react";
import CollapsibleRace from "@/components/CollapsibleRace";
import RaceCard from "@/components/RaceCard";

const h = [
  {
    Event: {
      type: "past",
      FeedId: 12,
      EventId: "8563859",
      ParentEventId: null,
      TypeValue: 2,
      BettingLayoutEnumValue: 0,
      TypeName: "PlatinumHounds",
      SubTypeName: "None",
      StartDateTimeAsWords: "2025/12/25 15:21:00",
      StartTimeAsWords: "15:21",
      FinishTimeAsWords: "15:21",
      Race: {
        Name: "Crawford Park (Night)",
        PlacePaysOn: 3,
        Distance: 720,
        Entries: [
          {
            FeedId: 583,
            Draw: 1,
            Name: "Mandolin",
            Finish: null,
            Form: "3,6,4,5,8",
            LBW: "2.9;5.2;2.7;5.9;6.5",
            StarRating: 40,
            RacesSinceWin: 5,
            RacesSincePlace: 4,
            WinOdds: 27.17,
            PlaceOdds: 3.83,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "39",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 5.2, 2.7, 5.9, 6.5],
            InverseRating: 50,
            Description: "1. Mandolin",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 583,
            HasErrorOccured: false,
            ErrorMessage: null,
            RunsSinceLast: "1.3005",
          },
          {
            FeedId: 425,
            Draw: 2,
            Name: "Crazy Eights",
            Finish: null,
            Form: "3,7,2,4,4",
            LBW: "2.1;6.6;2.9;2.7;4.0",
            StarRating: 38,
            RacesSinceWin: 32,
            RacesSincePlace: 2,
            WinOdds: 17.13,
            PlaceOdds: 3.14,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "29",
            DisplayFinish: 99,
            LBWAsArray: [2.1, 6.6, 2.9, 2.7, 4],
            InverseRating: 52,
            Description: "2. Crazy Eights",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 425,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 238,
            Draw: 3,
            Name: "Hotspur",
            Finish: null,
            Form: "3,4,3,6,7",
            LBW: "1.7;4.6;2.2;5.5;6.6",
            StarRating: 44,
            RacesSinceWin: 7,
            RacesSincePlace: 2,
            WinOdds: 7.55,
            PlaceOdds: 2.24,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "16",
            DisplayFinish: 99,
            LBWAsArray: [1.7, 4.6, 2.2, 5.5, 6.6],
            InverseRating: 45,
            Description: "3. Hotspur",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 238,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 124,
            Draw: 4,
            Name: "Max Million",
            Finish: null,
            Form: "3,7,5,1,2",
            LBW: "1.9;6.0;4.2;0;0.7",
            StarRating: 62,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 4.23,
            PlaceOdds: 1.77,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "09",
            DisplayFinish: 99,
            LBWAsArray: [1.9, 6, 4.2, 0, 0.7],
            InverseRating: 22,
            Description: "4. Max Million",
            Favorite: 2,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 124,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 22,
            Draw: 5,
            Name: "Onion Marmalade",
            Finish: null,
            Form: "4,3,1,1,2",
            LBW: "2.9;1.9;0;0;0.9",
            StarRating: 67,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 2.56,
            PlaceOdds: 1.42,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "02",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 1.9, 0, 0, 0.9],
            InverseRating: 16,
            Description: "5. Onion Marmalade",
            Favorite: 1,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 22,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 364,
            Draw: 6,
            Name: "Just a Ripple",
            Finish: null,
            Form: "7,2,5,2,2",
            LBW: "6.1;0.9;4.4;2.5;1.1",
            StarRating: 51,
            RacesSinceWin: 10,
            RacesSincePlace: 0,
            WinOdds: 13.49,
            PlaceOdds: 2.85,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "25",
            DisplayFinish: 99,
            LBWAsArray: [6.1, 0.9, 4.4, 2.5, 1.1],
            InverseRating: 36,
            Description: "6. Just a Ripple",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 364,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 195,
            Draw: 7,
            Name: "Orkney Snorkney",
            Finish: null,
            Form: "2,3,3,3,8",
            LBW: "1.0;2.0;2.6;1.5;7.3",
            StarRating: 58,
            RacesSinceWin: 6,
            RacesSincePlace: 1,
            WinOdds: 6.08,
            PlaceOdds: 2.05,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "13",
            DisplayFinish: 99,
            LBWAsArray: [1, 2, 2.6, 1.5, 7.3],
            InverseRating: 28,
            Description: "7. Orkney Snorkney",
            Favorite: 3,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 195,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 524,
            Draw: 8,
            Name: "Access Granted",
            Finish: null,
            Form: "6,8,2,1,3",
            LBW: "4.9;7.2;0.8;0;2.3",
            StarRating: 40,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 23.43,
            PlaceOdds: 3.59,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "35",
            DisplayFinish: 99,
            LBWAsArray: [4.9, 7.2, 0.8, 0, 2.3],
            InverseRating: 50,
            Description: "8. Access Granted",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 524,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
        ],
        Result: null,
        KenoFeedID: 0,
        KenoResult: null,
        NoOfParticipants: null,
        ID: 0,
        HasErrorOccured: false,
        ErrorMessage: null,
      },
      Boxing: null,
      PlayerVsPlayer: null,
      Archery: null,
      FootballMatch: null,
      FootballTournamentMatch: null,
      RacingRouletteV2: null,
      SpinAndWin: null,
      FootballLeagueWeek: null,
      IceHockeyMatch: null,
      FinalHoopsMatch: null,
      PrimaryMarkets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      PrimaryMarketsStore: [],
      Markets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "OE",
          Name: "Odd/Even",
          Abbreviation: null,
          ClassValue: 134,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "O",
              Odds: 1.44,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "O",
              DisplayDescription: "Odd",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "E",
              Odds: 2.54,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "E",
              DisplayDescription: "Even",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingOddEven",
          MarketClassID: 134,
          DisplayDescription: "Odd/Even",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 134,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingOddEven",
          ID: "12-2-8563859-134",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "HL",
          Name: "High/Low",
          Abbreviation: null,
          ClassValue: 135,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "H",
              Odds: 1.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "H",
              DisplayDescription: "High",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "L",
              Odds: 2.25,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "L",
              DisplayDescription: "Low",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingHighLow",
          MarketClassID: 135,
          DisplayDescription: "High/Low",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 6,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 135,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingHighLow",
          ID: "12-2-8563859-135",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 3,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Forecast",
          MarketClassID: 3,
          DisplayDescription: "1st Two In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 3,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Forecast",
          ID: "12-2-8563859-3",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 5,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Tricast",
          MarketClassID: 5,
          DisplayDescription: "1st Three In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 3,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 5,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Tricast",
          ID: "12-2-8563859-5",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 4,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseForecast",
          MarketClassID: 4,
          DisplayDescription: "1st Two Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 4,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseForecast",
          ID: "12-2-8563859-4",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 6,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseTricast",
          MarketClassID: 6,
          DisplayDescription: "1st Three Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 4,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 6,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseTricast",
          ID: "12-2-8563859-6",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 133,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Swinger",
          MarketClassID: 133,
          DisplayDescription: "Swinger",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 133,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Swinger",
          ID: "12-2-8563859-133",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      Events: null,
      AdjustedStartTime: "/Date(1766665260097)/",
      AdjustedFinishTime: "/Date(1766665315097)/",
      BetCloseTime: "/Date(1766665260097)/",
      Number: 2076,
      StatusValue: 1,
      IsFinished: false,
      FeedGuid: "00000000-0000-0000-0000-000000000000",
      CacheExpiryDate: null,
      CacheExpiryDateAsWords: "",
      EstimatedFinishTime: "/Date(1766665335097)/",
      IsNow: false,
      IsNext: false,
      HasBeenProcessed: false,
      GameProviderGUID: "00000000-0000-0000-0000-000000000000",
      IceHockeyLeagueWeek: null,
      FinalHoopsLeagueWeek: null,
      FootballTournamentLeagueWeek: null,
      CricketMatch: null,
      LuckyRacing: null,
      Animalitos: null,
      LegendsIceHockeyGoalShot: null,
      ID: "12-2-8563859",
      HasErrorOccured: false,
      ErrorMessage: null,
    },
  },
  {
    Event: {
      type: "live",
      FeedId: 12,
      EventId: "8563859",
      ParentEventId: null,
      TypeValue: 2,
      BettingLayoutEnumValue: 0,
      TypeName: "PlatinumHounds",
      SubTypeName: "None",
      StartDateTimeAsWords: "2025/12/25 15:21:00",
      StartTimeAsWords: "15:21",
      FinishTimeAsWords: "15:21",
      Race: {
        Name: "Crawford Park (Night)",
        PlacePaysOn: 3,
        Distance: 720,
        Entries: [
          {
            FeedId: 583,
            Draw: 1,
            Name: "Mandolin",
            Finish: null,
            Form: "3,6,4,5,8",
            LBW: "2.9;5.2;2.7;5.9;6.5",
            StarRating: 40,
            RacesSinceWin: 5,
            RacesSincePlace: 4,
            WinOdds: 27.17,
            PlaceOdds: 3.83,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "39",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 5.2, 2.7, 5.9, 6.5],
            InverseRating: 50,
            Description: "1. Mandolin",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 583,
            HasErrorOccured: false,
            ErrorMessage: null,
            RunsSinceLast: "1.3005",
          },
          {
            FeedId: 425,
            Draw: 2,
            Name: "Crazy Eights",
            Finish: null,
            Form: "3,7,2,4,4",
            LBW: "2.1;6.6;2.9;2.7;4.0",
            StarRating: 38,
            RacesSinceWin: 32,
            RacesSincePlace: 2,
            WinOdds: 17.13,
            PlaceOdds: 3.14,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "29",
            DisplayFinish: 99,
            LBWAsArray: [2.1, 6.6, 2.9, 2.7, 4],
            InverseRating: 52,
            Description: "2. Crazy Eights",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 425,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 238,
            Draw: 3,
            Name: "Hotspur",
            Finish: null,
            Form: "3,4,3,6,7",
            LBW: "1.7;4.6;2.2;5.5;6.6",
            StarRating: 44,
            RacesSinceWin: 7,
            RacesSincePlace: 2,
            WinOdds: 7.55,
            PlaceOdds: 2.24,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "16",
            DisplayFinish: 99,
            LBWAsArray: [1.7, 4.6, 2.2, 5.5, 6.6],
            InverseRating: 45,
            Description: "3. Hotspur",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 238,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 124,
            Draw: 4,
            Name: "Max Million",
            Finish: null,
            Form: "3,7,5,1,2",
            LBW: "1.9;6.0;4.2;0;0.7",
            StarRating: 62,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 4.23,
            PlaceOdds: 1.77,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "09",
            DisplayFinish: 99,
            LBWAsArray: [1.9, 6, 4.2, 0, 0.7],
            InverseRating: 22,
            Description: "4. Max Million",
            Favorite: 2,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 124,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 22,
            Draw: 5,
            Name: "Onion Marmalade",
            Finish: null,
            Form: "4,3,1,1,2",
            LBW: "2.9;1.9;0;0;0.9",
            StarRating: 67,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 2.56,
            PlaceOdds: 1.42,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "02",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 1.9, 0, 0, 0.9],
            InverseRating: 16,
            Description: "5. Onion Marmalade",
            Favorite: 1,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 22,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 364,
            Draw: 6,
            Name: "Just a Ripple",
            Finish: null,
            Form: "7,2,5,2,2",
            LBW: "6.1;0.9;4.4;2.5;1.1",
            StarRating: 51,
            RacesSinceWin: 10,
            RacesSincePlace: 0,
            WinOdds: 13.49,
            PlaceOdds: 2.85,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "25",
            DisplayFinish: 99,
            LBWAsArray: [6.1, 0.9, 4.4, 2.5, 1.1],
            InverseRating: 36,
            Description: "6. Just a Ripple",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 364,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 195,
            Draw: 7,
            Name: "Orkney Snorkney",
            Finish: null,
            Form: "2,3,3,3,8",
            LBW: "1.0;2.0;2.6;1.5;7.3",
            StarRating: 58,
            RacesSinceWin: 6,
            RacesSincePlace: 1,
            WinOdds: 6.08,
            PlaceOdds: 2.05,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "13",
            DisplayFinish: 99,
            LBWAsArray: [1, 2, 2.6, 1.5, 7.3],
            InverseRating: 28,
            Description: "7. Orkney Snorkney",
            Favorite: 3,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 195,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 524,
            Draw: 8,
            Name: "Access Granted",
            Finish: null,
            Form: "6,8,2,1,3",
            LBW: "4.9;7.2;0.8;0;2.3",
            StarRating: 40,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 23.43,
            PlaceOdds: 3.59,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "35",
            DisplayFinish: 99,
            LBWAsArray: [4.9, 7.2, 0.8, 0, 2.3],
            InverseRating: 50,
            Description: "8. Access Granted",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 524,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
        ],
        Result: null,
        KenoFeedID: 0,
        KenoResult: null,
        NoOfParticipants: null,
        ID: 0,
        HasErrorOccured: false,
        ErrorMessage: null,
      },
      Boxing: null,
      PlayerVsPlayer: null,
      Archery: null,
      FootballMatch: null,
      FootballTournamentMatch: null,
      RacingRouletteV2: null,
      SpinAndWin: null,
      FootballLeagueWeek: null,
      IceHockeyMatch: null,
      FinalHoopsMatch: null,
      PrimaryMarkets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      PrimaryMarketsStore: [],
      Markets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "OE",
          Name: "Odd/Even",
          Abbreviation: null,
          ClassValue: 134,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "O",
              Odds: 1.44,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "O",
              DisplayDescription: "Odd",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "E",
              Odds: 2.54,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "E",
              DisplayDescription: "Even",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingOddEven",
          MarketClassID: 134,
          DisplayDescription: "Odd/Even",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 134,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingOddEven",
          ID: "12-2-8563859-134",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "HL",
          Name: "High/Low",
          Abbreviation: null,
          ClassValue: 135,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "H",
              Odds: 1.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "H",
              DisplayDescription: "High",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "L",
              Odds: 2.25,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "L",
              DisplayDescription: "Low",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingHighLow",
          MarketClassID: 135,
          DisplayDescription: "High/Low",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 6,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 135,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingHighLow",
          ID: "12-2-8563859-135",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 3,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Forecast",
          MarketClassID: 3,
          DisplayDescription: "1st Two In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 3,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Forecast",
          ID: "12-2-8563859-3",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 5,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Tricast",
          MarketClassID: 5,
          DisplayDescription: "1st Three In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 3,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 5,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Tricast",
          ID: "12-2-8563859-5",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 4,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseForecast",
          MarketClassID: 4,
          DisplayDescription: "1st Two Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 4,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseForecast",
          ID: "12-2-8563859-4",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 6,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseTricast",
          MarketClassID: 6,
          DisplayDescription: "1st Three Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 4,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 6,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseTricast",
          ID: "12-2-8563859-6",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 133,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Swinger",
          MarketClassID: 133,
          DisplayDescription: "Swinger",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 133,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Swinger",
          ID: "12-2-8563859-133",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      Events: null,
      AdjustedStartTime: "/Date(1766665260097)/",
      AdjustedFinishTime: "/Date(1766665315097)/",
      BetCloseTime: "/Date(1766665260097)/",
      Number: 2076,
      StatusValue: 1,
      IsFinished: false,
      FeedGuid: "00000000-0000-0000-0000-000000000000",
      CacheExpiryDate: null,
      CacheExpiryDateAsWords: "",
      EstimatedFinishTime: "/Date(1766665335097)/",
      IsNow: false,
      IsNext: false,
      HasBeenProcessed: false,
      GameProviderGUID: "00000000-0000-0000-0000-000000000000",
      IceHockeyLeagueWeek: null,
      FinalHoopsLeagueWeek: null,
      FootballTournamentLeagueWeek: null,
      CricketMatch: null,
      LuckyRacing: null,
      Animalitos: null,
      LegendsIceHockeyGoalShot: null,
      ID: "12-5-8563859",
      HasErrorOccured: false,
      ErrorMessage: null,
    },
  },
  {
    Event: {
      type: "next",
      FeedId: 12,
      EventId: "8563859",
      ParentEventId: null,
      TypeValue: 2,
      BettingLayoutEnumValue: 0,
      TypeName: "PlatinumHounds",
      SubTypeName: "None",
      StartDateTimeAsWords: "2025/12/25 15:21:00",
      StartTimeAsWords: "15:21",
      FinishTimeAsWords: "15:21",
      Race: {
        Name: "Crawford Park (Night)",
        PlacePaysOn: 3,
        Distance: 720,
        Entries: [
          {
            FeedId: 583,
            Draw: 1,
            Name: "Mandolin",
            Finish: null,
            Form: "3,6,4,5,8",
            LBW: "2.9;5.2;2.7;5.9;6.5",
            StarRating: 40,
            RacesSinceWin: 5,
            RacesSincePlace: 4,
            WinOdds: 27.17,
            PlaceOdds: 3.83,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "39",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 5.2, 2.7, 5.9, 6.5],
            InverseRating: 50,
            Description: "1. Mandolin",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 583,
            HasErrorOccured: false,
            ErrorMessage: null,
            RunsSinceLast: "1.3005",
          },
          {
            FeedId: 425,
            Draw: 2,
            Name: "Crazy Eights",
            Finish: null,
            Form: "3,7,2,4,4",
            LBW: "2.1;6.6;2.9;2.7;4.0",
            StarRating: 38,
            RacesSinceWin: 32,
            RacesSincePlace: 2,
            WinOdds: 17.13,
            PlaceOdds: 3.14,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "29",
            DisplayFinish: 99,
            LBWAsArray: [2.1, 6.6, 2.9, 2.7, 4],
            InverseRating: 52,
            Description: "2. Crazy Eights",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 425,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 238,
            Draw: 3,
            Name: "Hotspur",
            Finish: null,
            Form: "3,4,3,6,7",
            LBW: "1.7;4.6;2.2;5.5;6.6",
            StarRating: 44,
            RacesSinceWin: 7,
            RacesSincePlace: 2,
            WinOdds: 7.55,
            PlaceOdds: 2.24,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "16",
            DisplayFinish: 99,
            LBWAsArray: [1.7, 4.6, 2.2, 5.5, 6.6],
            InverseRating: 45,
            Description: "3. Hotspur",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 238,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 124,
            Draw: 4,
            Name: "Max Million",
            Finish: null,
            Form: "3,7,5,1,2",
            LBW: "1.9;6.0;4.2;0;0.7",
            StarRating: 62,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 4.23,
            PlaceOdds: 1.77,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "09",
            DisplayFinish: 99,
            LBWAsArray: [1.9, 6, 4.2, 0, 0.7],
            InverseRating: 22,
            Description: "4. Max Million",
            Favorite: 2,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 124,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 22,
            Draw: 5,
            Name: "Onion Marmalade",
            Finish: null,
            Form: "4,3,1,1,2",
            LBW: "2.9;1.9;0;0;0.9",
            StarRating: 67,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 2.56,
            PlaceOdds: 1.42,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "02",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 1.9, 0, 0, 0.9],
            InverseRating: 16,
            Description: "5. Onion Marmalade",
            Favorite: 1,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 22,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 364,
            Draw: 6,
            Name: "Just a Ripple",
            Finish: null,
            Form: "7,2,5,2,2",
            LBW: "6.1;0.9;4.4;2.5;1.1",
            StarRating: 51,
            RacesSinceWin: 10,
            RacesSincePlace: 0,
            WinOdds: 13.49,
            PlaceOdds: 2.85,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "25",
            DisplayFinish: 99,
            LBWAsArray: [6.1, 0.9, 4.4, 2.5, 1.1],
            InverseRating: 36,
            Description: "6. Just a Ripple",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 364,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 195,
            Draw: 7,
            Name: "Orkney Snorkney",
            Finish: null,
            Form: "2,3,3,3,8",
            LBW: "1.0;2.0;2.6;1.5;7.3",
            StarRating: 58,
            RacesSinceWin: 6,
            RacesSincePlace: 1,
            WinOdds: 6.08,
            PlaceOdds: 2.05,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "13",
            DisplayFinish: 99,
            LBWAsArray: [1, 2, 2.6, 1.5, 7.3],
            InverseRating: 28,
            Description: "7. Orkney Snorkney",
            Favorite: 3,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 195,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 524,
            Draw: 8,
            Name: "Access Granted",
            Finish: null,
            Form: "6,8,2,1,3",
            LBW: "4.9;7.2;0.8;0;2.3",
            StarRating: 40,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 23.43,
            PlaceOdds: 3.59,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "35",
            DisplayFinish: 99,
            LBWAsArray: [4.9, 7.2, 0.8, 0, 2.3],
            InverseRating: 50,
            Description: "8. Access Granted",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 524,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
        ],
        Result: null,
        KenoFeedID: 0,
        KenoResult: null,
        NoOfParticipants: null,
        ID: 0,
        HasErrorOccured: false,
        ErrorMessage: null,
      },
      Boxing: null,
      PlayerVsPlayer: null,
      Archery: null,
      FootballMatch: null,
      FootballTournamentMatch: null,
      RacingRouletteV2: null,
      SpinAndWin: null,
      FootballLeagueWeek: null,
      IceHockeyMatch: null,
      FinalHoopsMatch: null,
      PrimaryMarkets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      PrimaryMarketsStore: [],
      Markets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "OE",
          Name: "Odd/Even",
          Abbreviation: null,
          ClassValue: 134,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "O",
              Odds: 1.44,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "O",
              DisplayDescription: "Odd",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "E",
              Odds: 2.54,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "E",
              DisplayDescription: "Even",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingOddEven",
          MarketClassID: 134,
          DisplayDescription: "Odd/Even",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 134,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingOddEven",
          ID: "12-2-8563859-134",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "HL",
          Name: "High/Low",
          Abbreviation: null,
          ClassValue: 135,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "H",
              Odds: 1.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "H",
              DisplayDescription: "High",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "L",
              Odds: 2.25,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "L",
              DisplayDescription: "Low",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingHighLow",
          MarketClassID: 135,
          DisplayDescription: "High/Low",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 6,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 135,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingHighLow",
          ID: "12-2-8563859-135",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 3,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Forecast",
          MarketClassID: 3,
          DisplayDescription: "1st Two In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 3,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Forecast",
          ID: "12-2-8563859-3",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 5,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Tricast",
          MarketClassID: 5,
          DisplayDescription: "1st Three In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 3,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 5,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Tricast",
          ID: "12-2-8563859-5",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 4,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseForecast",
          MarketClassID: 4,
          DisplayDescription: "1st Two Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 4,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseForecast",
          ID: "12-2-8563859-4",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 6,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseTricast",
          MarketClassID: 6,
          DisplayDescription: "1st Three Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 4,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 6,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseTricast",
          ID: "12-2-8563859-6",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 133,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Swinger",
          MarketClassID: 133,
          DisplayDescription: "Swinger",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 133,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Swinger",
          ID: "12-2-8563859-133",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      Events: null,
      AdjustedStartTime: "/Date(1766665260097)/",
      AdjustedFinishTime: "/Date(1766665315097)/",
      BetCloseTime: "/Date(1766665260097)/",
      Number: 2076,
      StatusValue: 1,
      IsFinished: false,
      FeedGuid: "00000000-0000-0000-0000-000000000000",
      CacheExpiryDate: null,
      CacheExpiryDateAsWords: "",
      EstimatedFinishTime: "/Date(1766665335097)/",
      IsNow: false,
      IsNext: false,
      HasBeenProcessed: false,
      GameProviderGUID: "00000000-0000-0000-0000-000000000000",
      IceHockeyLeagueWeek: null,
      FinalHoopsLeagueWeek: null,
      FootballTournamentLeagueWeek: null,
      CricketMatch: null,
      LuckyRacing: null,
      Animalitos: null,
      LegendsIceHockeyGoalShot: null,
      ID: "12-3-8563859",
      HasErrorOccured: false,
      ErrorMessage: null,
    },
  },
  {
    Event: {
      type: "upcomming",
      FeedId: 12,
      EventId: "8563859",
      ParentEventId: null,
      TypeValue: 2,
      BettingLayoutEnumValue: 0,
      TypeName: "PlatinumHounds",
      SubTypeName: "None",
      StartDateTimeAsWords: "2025/12/25 15:21:00",
      StartTimeAsWords: "15:21",
      FinishTimeAsWords: "15:21",
      Race: {
        Name: "Crawford Park (Night)",
        PlacePaysOn: 3,
        Distance: 720,
        Entries: [
          {
            FeedId: 583,
            Draw: 1,
            Name: "Mandolin",
            Finish: null,
            Form: "3,6,4,5,8",
            LBW: "2.9;5.2;2.7;5.9;6.5",
            StarRating: 40,
            RacesSinceWin: 5,
            RacesSincePlace: 4,
            WinOdds: 27.17,
            PlaceOdds: 3.83,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "39",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 5.2, 2.7, 5.9, 6.5],
            InverseRating: 50,
            Description: "1. Mandolin",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 583,
            HasErrorOccured: false,
            ErrorMessage: null,
            RunsSinceLast: "1.3005",
          },
          {
            FeedId: 425,
            Draw: 2,
            Name: "Crazy Eights",
            Finish: null,
            Form: "3,7,2,4,4",
            LBW: "2.1;6.6;2.9;2.7;4.0",
            StarRating: 38,
            RacesSinceWin: 32,
            RacesSincePlace: 2,
            WinOdds: 17.13,
            PlaceOdds: 3.14,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "29",
            DisplayFinish: 99,
            LBWAsArray: [2.1, 6.6, 2.9, 2.7, 4],
            InverseRating: 52,
            Description: "2. Crazy Eights",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 425,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 238,
            Draw: 3,
            Name: "Hotspur",
            Finish: null,
            Form: "3,4,3,6,7",
            LBW: "1.7;4.6;2.2;5.5;6.6",
            StarRating: 44,
            RacesSinceWin: 7,
            RacesSincePlace: 2,
            WinOdds: 7.55,
            PlaceOdds: 2.24,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "16",
            DisplayFinish: 99,
            LBWAsArray: [1.7, 4.6, 2.2, 5.5, 6.6],
            InverseRating: 45,
            Description: "3. Hotspur",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 238,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 124,
            Draw: 4,
            Name: "Max Million",
            Finish: null,
            Form: "3,7,5,1,2",
            LBW: "1.9;6.0;4.2;0;0.7",
            StarRating: 62,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 4.23,
            PlaceOdds: 1.77,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "09",
            DisplayFinish: 99,
            LBWAsArray: [1.9, 6, 4.2, 0, 0.7],
            InverseRating: 22,
            Description: "4. Max Million",
            Favorite: 2,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 124,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 22,
            Draw: 5,
            Name: "Onion Marmalade",
            Finish: null,
            Form: "4,3,1,1,2",
            LBW: "2.9;1.9;0;0;0.9",
            StarRating: 67,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 2.56,
            PlaceOdds: 1.42,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "02",
            DisplayFinish: 99,
            LBWAsArray: [2.9, 1.9, 0, 0, 0.9],
            InverseRating: 16,
            Description: "5. Onion Marmalade",
            Favorite: 1,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 22,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 364,
            Draw: 6,
            Name: "Just a Ripple",
            Finish: null,
            Form: "7,2,5,2,2",
            LBW: "6.1;0.9;4.4;2.5;1.1",
            StarRating: 51,
            RacesSinceWin: 10,
            RacesSincePlace: 0,
            WinOdds: 13.49,
            PlaceOdds: 2.85,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "25",
            DisplayFinish: 99,
            LBWAsArray: [6.1, 0.9, 4.4, 2.5, 1.1],
            InverseRating: 36,
            Description: "6. Just a Ripple",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 364,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 195,
            Draw: 7,
            Name: "Orkney Snorkney",
            Finish: null,
            Form: "2,3,3,3,8",
            LBW: "1.0;2.0;2.6;1.5;7.3",
            StarRating: 58,
            RacesSinceWin: 6,
            RacesSincePlace: 1,
            WinOdds: 6.08,
            PlaceOdds: 2.05,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "13",
            DisplayFinish: 99,
            LBWAsArray: [1, 2, 2.6, 1.5, 7.3],
            InverseRating: 28,
            Description: "7. Orkney Snorkney",
            Favorite: 3,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 195,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
          {
            FeedId: 524,
            Draw: 8,
            Name: "Access Granted",
            Finish: null,
            Form: "6,8,2,1,3",
            LBW: "4.9;7.2;0.8;0;2.3",
            StarRating: 40,
            RacesSinceWin: 1,
            RacesSincePlace: 0,
            WinOdds: 23.43,
            PlaceOdds: 3.59,
            ShowOdds: 0,
            SecondOdds: 0,
            ThirdOdds: 0,
            LastOdds: 0,
            Last3Odds: 0,
            Gender: null,
            Age: null,
            SilkImagePath: null,
            SilkNumber: "35",
            DisplayFinish: 99,
            LBWAsArray: [4.9, 7.2, 0.8, 0, 2.3],
            InverseRating: 50,
            Description: "8. Access Granted",
            Favorite: null,
            BallAllocations: null,
            BallsDrawn: null,
            ID: 524,
            HasErrorOccured: false,
            ErrorMessage: null,
          },
        ],
        Result: null,
        KenoFeedID: 0,
        KenoResult: null,
        NoOfParticipants: null,
        ID: 0,
        HasErrorOccured: false,
        ErrorMessage: null,
      },
      Boxing: null,
      PlayerVsPlayer: null,
      Archery: null,
      FootballMatch: null,
      FootballTournamentMatch: null,
      RacingRouletteV2: null,
      SpinAndWin: null,
      FootballLeagueWeek: null,
      IceHockeyMatch: null,
      FinalHoopsMatch: null,
      PrimaryMarkets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      PrimaryMarketsStore: [],
      Markets: [
        {
          FeedId: "Win",
          Name: "Win",
          Abbreviation: null,
          ClassValue: 1,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 27.17,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 17.13,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 7.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 4.23,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 2.56,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 13.49,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 6.08,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 23.43,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: true,
          ClassEnumValue: "Win",
          MarketClassID: 1,
          DisplayDescription: "Win",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 1,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Win",
          ID: "12-2-8563859-1",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "Place",
          Name: "Place",
          Abbreviation: null,
          ClassValue: 2,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: 1,
              Second: null,
              Third: null,
              FeedId: "1",
              Odds: 3.83,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "1. Mandolin",
              DisplayDescription: "1. Mandolin",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 2,
              Second: null,
              Third: null,
              FeedId: "2",
              Odds: 3.14,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "2. Crazy Eights",
              DisplayDescription: "2. Crazy Eights",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 3,
              Second: null,
              Third: null,
              FeedId: "3",
              Odds: 2.24,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "3. Hotspur",
              DisplayDescription: "3. Hotspur",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 4,
              Second: null,
              Third: null,
              FeedId: "4",
              Odds: 1.77,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "4. Max Million",
              DisplayDescription: "4. Max Million",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 5,
              Second: null,
              Third: null,
              FeedId: "5",
              Odds: 1.42,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "5. Onion Marmalade",
              DisplayDescription: "5. Onion Marmalade",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 6,
              Second: null,
              Third: null,
              FeedId: "6",
              Odds: 2.85,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "6. Just a Ripple",
              DisplayDescription: "6. Just a Ripple",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 7,
              Second: null,
              Third: null,
              FeedId: "7",
              Odds: 2.05,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "7. Orkney Snorkney",
              DisplayDescription: "7. Orkney Snorkney",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: 8,
              Second: null,
              Third: null,
              FeedId: "8",
              Odds: 3.59,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "8. Access Granted",
              DisplayDescription: "8. Access Granted",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Place",
          MarketClassID: 2,
          DisplayDescription: "Place",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 2,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Place",
          ID: "12-2-8563859-2",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "OE",
          Name: "Odd/Even",
          Abbreviation: null,
          ClassValue: 134,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "O",
              Odds: 1.44,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "O",
              DisplayDescription: "Odd",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "E",
              Odds: 2.54,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "E",
              DisplayDescription: "Even",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingOddEven",
          MarketClassID: 134,
          DisplayDescription: "Odd/Even",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 134,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingOddEven",
          ID: "12-2-8563859-134",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: "HL",
          Name: "High/Low",
          Abbreviation: null,
          ClassValue: 135,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "H",
              Odds: 1.55,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "H",
              DisplayDescription: "High",
              ExtraDescription: "",
              EntryID: null,
            },
            {
              First: -1,
              Second: null,
              Third: null,
              FeedId: "L",
              Odds: 2.25,
              IsWinner: false,
              Sort: 0,
              FeedDescription: "L",
              DisplayDescription: "Low",
              ExtraDescription: "",
              EntryID: null,
            },
          ],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "RacingHighLow",
          MarketClassID: 135,
          DisplayDescription: "High/Low",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 6,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 135,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "RacingHighLow",
          ID: "12-2-8563859-135",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 3,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Forecast",
          MarketClassID: 3,
          DisplayDescription: "1st Two In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 1,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 3,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Forecast",
          ID: "12-2-8563859-3",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 5,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Tricast",
          MarketClassID: 5,
          DisplayDescription: "1st Three In Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 3,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 5,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Tricast",
          ID: "12-2-8563859-5",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 4,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseForecast",
          MarketClassID: 4,
          DisplayDescription: "1st Two Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 2,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 4,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseForecast",
          ID: "12-2-8563859-4",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 6,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "ReverseTricast",
          MarketClassID: 6,
          DisplayDescription: "1st Three Any Order",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 4,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 6,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "ReverseTricast",
          ID: "12-2-8563859-6",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
        {
          FeedId: null,
          Name: null,
          Abbreviation: null,
          ClassValue: 133,
          ArcherySelections: [],
          BoxingSelections: [],
          InstantBetSelections: [],
          PlayerVsPlayerSelections: [],
          FootballMatchSelections: [],
          KenoSelections: [],
          LuckyLoot624Selections: [],
          RaceSelections: [],
          RacingRouletteSelections: [],
          SpinAndWinSelections: [],
          IceHockeyMatchSelections: [],
          FinalHoopsMatchSelections: [],
          CricketMatchSelections: [],
          MarketClassSelections: [],
          FootballTournamentMatchSelections: [],
          LuckyRacingSelections: [],
          AnimalitosSelections: [],
          LegendsIceHockeyGoalShotSelections: [],
          Split: null,
          IsClassAvailableForBetting: true,
          IsClassParticipants: false,
          ClassEnumValue: "Swinger",
          MarketClassID: 133,
          DisplayDescription: "Swinger",
          AmericanDisplayDescription: null,
          CssClass: null,
          BettingLayouts: [
            {
              EventTypeEnumValue: 2,
              MarketClassEventTypeID: 0,
              Position: 5,
              IsExpanded: false,
              BettingLayoutEnumValue: 1,
              MarketClassID: 133,
              ParentEventTypeEnumValue: null,
              LayoutPattern: null,
            },
          ],
          WinningSelectionID: null,
          WinningSelectionDescription: null,
          ResourceKey: "Swinger",
          ID: "12-2-8563859-133",
          HasErrorOccured: false,
          ErrorMessage: null,
        },
      ],
      Events: null,
      AdjustedStartTime: "/Date(1766665260097)/",
      AdjustedFinishTime: "/Date(1766665315097)/",
      BetCloseTime: "/Date(1766665260097)/",
      Number: 2076,
      StatusValue: 1,
      IsFinished: false,
      FeedGuid: "00000000-0000-0000-0000-000000000000",
      CacheExpiryDate: null,
      CacheExpiryDateAsWords: "",
      EstimatedFinishTime: "/Date(1766665335097)/",
      IsNow: false,
      IsNext: false,
      HasBeenProcessed: false,
      GameProviderGUID: "00000000-0000-0000-0000-000000000000",
      IceHockeyLeagueWeek: null,
      FinalHoopsLeagueWeek: null,
      FootballTournamentLeagueWeek: null,
      CricketMatch: null,
      LuckyRacing: null,
      Animalitos: null,
      LegendsIceHockeyGoalShot: null,
      ID: "12-4-8563859",
      HasErrorOccured: false,
      ErrorMessage: null,
    },
  },
];

export default function Home() {
  const [activeTab, setActiveTab] = useState("MAIN");

  const handleTabClick = (tab: string) => {
    setActiveTab(tab);
    console.log(`Tab: ${tab}`);
  };

  return (
    <main className="py-6 min-h-screen w-[70%]">
      <div className="ml-6 inline-flex overflow-hidden text-xs font-normal">
        {["MAIN", "ALT", "HEAD TO HEAD", "SUM"].map((tab, idx) => (
          <button
            key={tab}
            onClick={() => handleTabClick(tab)}
            className={`px-3 py-1 text-center text-gray-300 cursor-pointer font-semibold border border-[#36B250] ${
              activeTab === tab ? "bg-[#36B250] text-white" : "text-gray-600"
            } ${idx !== 0 ? "-ml-[1px]" : ""}`}
          >
            {tab}
          </button>
        ))}
      </div>

      <div className="ml-2 mt-2">
        {h.map((r) => (
          <CollapsibleRace
            key={r["Event"]["ID"]}
            title={r["Event"]["Race"]["Name"]}
            subtitle={r["Event"]["ID"]}
            horses={r["Event"]}
            type={r["Event"]["type"]}
            defaultOpen={r["Event"]["type"] === "live"}
          >
            <RaceCard
              horses={r["Event"]["Race"]["Entries"]}
              type={r["Event"]["type"]}
            />
          </CollapsibleRace>
        ))}
      </div>
    </main>
  );
}
